...
"""
Convert DistortionNet 3-patch model to TFLite format with INT8 quantization.

This script converts the DistortionNet model with batch_size=3 (processing 3 patches
at a time) to TFLite format with INT8 quantization.

Usage:
    python convert_distortion_3patch_int8.py
"""
...
def convert_distortion_net_3patch_int8(output_dir):
    """Convert DistortionNet 3-patch model to TFLite with INT8 quantization.
    ...
    """
    print("\n" + "="*70)
    print("Converting DistortionNet 3-Patch Model to TFLite with INT8 Quantization")
    print("="*70)
    ...
    print(f"Expected output shape: [3, 8, 8, 128] (3 individual patch features)")
    print(f"Note: Aggregation to [1, 8, 24, 128] is done in application code")
    ...
    # Save model
    output_file = os.path.join(output_dir, "distortion_net_3patch_int8.tflite")
    edge_model.export(output_file)
    
    file_size = os.path.getsize(output_file) / (1024 * 1024)
    print(f"\n✓ DistortionNet 3-patch (INT8) saved to: {output_file}")
    print(f"  File size: {file_size:.2f} MB")
    
    # Compare with FLOAT32 version if it exists
    float32_file = os.path.join(output_dir, "distortion_net_3patch.tflite")
...
def main():
    ...
    print("\n" + "="*70)
    print("DistortionNet 3-Patch Model to TFLite Conversion (INT8)")
    print("="*70)
    print(f"Output directory: {output_dir}")
    
    try:
        output_file = convert_distortion_net_3patch_int8(output_dir)
        
        print("\n" + "="*70)
        print("Conversion Summary")
        print("="*70)
        print(f"\n✓ Successfully converted model:")
        print(f"  - distortion_net_3patch_int8: {output_file}")
